...
namespace Net.Codecrete.QrCodeGenerator.Analyzer
{
    public class QrCodeDrawing
    {
        ...
        {
            return CreateDrawingInternal(qrCode, size, borderWidth, foreground, background, null);
        }

        internal static DrawingImage CreateDrawing(QrCode qrCode, double size, int borderWidth, IEnumerable<Rectangle>? highlights)
        {
            return CreateDrawingInternal(qrCode, size, borderWidth, Colors.Black, Colors.White, highlights);
        }

        private static DrawingImage CreateDrawingInternal(QrCode qrCode, double size, int borderWidth, Color foreground, Color? background, IEnumerable<Rectangle>? highlights)
        {
            ...
            using (var drawingContext = group.Open())
            {
                ...
                if (highlights != null)
                {
                    var pen = new Pen(new SolidColorBrush(Colors.Red), 0.15);
                    pen.Freeze();
                    foreach (var b in highlights)
                    {
                        drawingContext.DrawRectangle(null, pen, new Rect(b.X + borderWidth, b.Y + borderWidth, b.Width, b.Height));
                    }
                }
                drawingContext.Pop();
            }
        ...
        public static void Draw(QrCode qrCode, DrawingContext drawingContext, int borderWidth, Brush foreground, Brush? background)
        {
            ...
            foreach (var rect in qrCode.ToRectangles())
            {
                ...
                drawingContext.DrawRectangle(foreground, null, r);
            }
        }

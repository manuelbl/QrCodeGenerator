...
namespace Net.Codecrete.QrCodeGenerator.Test
{
    public class SegmentCompactionTest
    {
        ...
        public void TestCompaction(string text, int version, ECI eci, int expectedLength)
        {
            ...
            var segments = SegmentCompaction.BuildSegments(new ArraySegment<byte>(utf8Data), version, eci.Equals(ECI.ShiftJIS));
        ...
        {
            internal TestCase(string text, int version, ECI eci, int expectedLength)
            {
                Text = text;
                Version = version;
                Eci = eci;

...
2. **`SegmentCompaction`** — picks the cheapest per-byte mode (numeric > alphanumeric > Kanji > binary), groups consecutive bytes into blocks, then assigns each block the mode of the segment it ends up in by a dynamic program over the blocks (costs in sixths of a bit, rounded up at segment ends), so the bit stream is the shortest possible. Header cost depends on `version` only through the count-indicator widths, which are constant within the version groups 1–9 / 10–26 / 27–40. `QrCode.EncodeTextAdvanced` and `EncodeBinary` therefore build the blocks once (`SegmentSource`), derive the segments per group in ascending order, and take the first group whose segments fit; groups the data cannot fit into are skipped by a lower bound. This is what produces the "smallest possible QR code" claim.
...
Two orderings are deliberately ordered by profiling data (see `QrCodeGeneratorProfiling/README.md`), not arbitrary: `MatrixEncoder.PatternEvaluationOrder` (evaluate likely-best masks first to tighten the early-stop bound) and the rule order inside `Penalty.Calculate`. Reordering them changes performance, not output. Per-version results are cached in `ConcurrentDictionary` instances — `FixedPatterns` caches the drawn fixed patterns and the payload-area map; `MatrixEncoder` caches the data-mask patterns and the payload target table; `Penalty` caches the per-size edge masks. Cached `BitMatrix` instances are shared and must not be mutated (callers `Copy()` first). The factory each `GetOrAdd` passes is held in a `static readonly Func<>` field rather than written as a method group, which under the project's `LangVersion` would allocate a delegate on every lookup.

Allocation on the encode path is kept to what the result needs: `Codewords.BuildData` hands out the `BitStream`'s own buffer instead of copying into a second array of the same length, `BitMatrix.Transpose` uses `[ThreadStatic]` scratch blocks, and `ECI` caches the `Encoding` per code page (`Encoding.GetEncoding` cannot cache one itself, since the custom fallbacks force it to clone).
...
- `RectangleBuilder` merges adjacent dark modules into the largest rectangles (greedy, non-overlapping, union == dark modules) to shrink output. `QrCode.ToRectangles` exposes the list publicly (as `QrRectangle`s, in `GetModule` coordinates with no border), for applications drawing the QR code shape by shape.
- `OutlineBuilder` traces the boundary of the dark modules as closed polygons (`QrPolygon`s of `QrPoint`s, on the grid of module corners, no border), exposed by `QrCode.ToOutlines`. It walks each boundary loop with the dark module on its right, so group loops come out clockwise and hole loops counterclockwise, and turns right at a corner where two dark modules touch only diagonally — dark modules are 4-connected, light areas 8-connected. It is the geometry `SvgBuilder` (SVG document + SVG/XAML path) emits, one closed sub-path per polygon with the border added at emit time: a single filled path has no seams between adjacent shapes, and the winding makes it fill correctly under the nonzero *and* the even-odd rule, so no fill rule has to be specified. The SVG document draws that path with `shape-rendering="crispEdges"`, and omits the background rectangle when the background color is `null`. `BmpBuilder` (BMP) and `PngBuilder` (PNG) take the finished modules directly. `QrCode` delegates to all of them.
